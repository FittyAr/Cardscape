...
public sealed record CreateDashcardRequest(int Kind, string Title, string? ConfigurationJson);
...
public static class DashboardsEndpoints
{
    ...
    {
        var group = app.MapGroup("/api/boards/{boardId:guid}/dashcards").RequireAuthorization().WithTags("Dashboards");
        ...
        group.MapPost("/", async (
            Guid boardId,
            CreateDashcardRequest body,
            IMessageBus bus,
            CancellationToken ct) =>
        {
            var result = await bus.InvokeAsync<Result<DashcardDto>>(
                new CreateDashcardCommand(boardId, body.Kind, body.Title, body.ConfigurationJson), ct);
            return result.IsSuccess
                ? Results.Created($"/api/boards/{boardId}/dashcards", result.Value)
                : MapError(result.Error);
        });

        group.MapDelete("/{dashcardId:guid}", async (
            Guid boardId,
            Guid dashcardId,
            IMessageBus bus,
            CancellationToken ct) =>
