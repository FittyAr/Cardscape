    ...
    public static IEndpointRouteBuilder MapExternalLoginEndpoints(this IEndpointRouteBuilder app)
    {
            ...
            CancellationToken ct) =>
        {
            ...
            string redirect = configuration["Cardscape:Web:ExternalLoginRedirectUrl"]
                ...
                ?? "/oauth/callback";
            string fragment =
                $"access_token={Uri.EscapeDataString(auth.Value.AccessToken)}"
                + $"&refresh_token={Uri.EscapeDataString(auth.Value.RefreshToken)}"
                + $"&expires_at={Uri.EscapeDataString(auth.Value.AccessTokenExpiresAt.ToString("O"))}"
