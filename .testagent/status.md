...
- [x] `OAuthApp` persists only the eight-character public prefix alongside the one-way SHA-256 hash; cleartext remains issuance-only.
- [x] SQLite, PostgreSQL and MySQL migrations were generated by EF Core and all three snapshots report no pending model changes.
- [x] `Register_Then_List_Adds_OAuthApp_To_Owner` asserts list prefix equals both registration prefix and `ClientSecret[..8]`; OAuth App endpoint slice passes 4/4.
- [x] Assertion/pseudo-mutation review: deriving from `ClientSecretHash`, omitting persistence, returning another prefix or changing issuance slicing breaks an exact equality assertion.
# Observability and health checks — 2026-09-10

- Implementation: complete; real live/ready mappings and host telemetry pipelines added.
