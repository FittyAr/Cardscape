...
namespace RadzenBlazorApp1.Client.Pages
{
    public partial class Contacts
    {
        ...
        protected RadzenDataGrid<Contact> grid0;
        protected int count;
        ...
        protected async Task Grid0LoadData(LoadDataArgs args)
        {
            ...
            try
            {
                ...
                var result = await CrmDBService.GetContacts(filter: filters.ToODataFilterString<Contact>(LogicalFilterOperator.And, grid0.FilterCaseSensitivity), orderby: $"{args.OrderBy}", top: args.Top, skip: args.Skip, count:args.Top != null && args.Skip != null);
        ...
        protected async Task EditRow(Contact args)
        {
            await DialogService.OpenAsync<EditContact>("Edit Contact", new Dictionary<string, object> { {"Id", args.Id} });
            await grid0.Reload();
        }

        protected async Task GridDeleteButtonClick(MouseEventArgs args, Contact contact)
        {
            try
            {
                if (await DialogService.Confirm("Are you sure you want to delete this record?") == true)
                {
                    var deleteResult = await CrmDBService.DeleteContact(id:contact.Id);
        ...
        protected async Task ExportClick(RadzenSplitButtonItem args)
        {
            if (args?.Value == "csv")
            {
                await CrmDBService.ExportContactsToCSV(new Query
                { 
                    Filter = filters.ToFilterString<Contact>(LogicalFilterOperator.And, grid0.FilterCaseSensitivity), 
            ...
            {
                await CrmDBService.ExportContactsToExcel(new Query
                { 
                    Filter = filters.ToFilterString<Contact>(LogicalFilterOperator.And, grid0.FilterCaseSensitivity), 
                    OrderBy = $"{grid0.Query.OrderBy}", 
                    Expand = "", 
