# Implementation plan

> The phased delivery plan for Cardscape, derived from
> [`00-feature-inventory.md`](00-feature-inventory.md).
> Phases are ordered; the first three cover the MVP, the next
> two cover the bulk of the  feature surface, and the last
> is the long tail.

## 0. Status

| Phase | Status | Notes |
|---|---|---|
| 0 — Solution scaffold | **DONE** | Commits `cd2170b` (initial), `dc8e68b` (testing strategy). |
| 1 — MVP | not started | Target: end of August 2026 (4–6 weeks from now). |
| 2 — Core features | not started | Target: end of October 2026. |
| 3 — Power-ups & Automation | not started | Target: end of December 2026. |
| 4 — Enterprise & AI | not started | Target: end of Q1 2027. |
| 5 — Polish & scale | not started | ongoing, no fixed target. |

The dates are **aspirational** and depend on contributor
bandwidth. Each phase ends with a tagged release
(`v0.1.0-mvp`, `v0.2.0-core`, etc.) and a self-hosted demo
deployable from a single `docker-compose up`.

---

## 1. Phase 0 — Solution scaffold (DONE)

**Commit:** `cd2170b` + `dc8e68b`

Deliverables:

- 10-project Clean Architecture skeleton in `src/` and
  `tests/`.
- Central Package Management on `Directory.Packages.props`.
- RPL-1.5 LICENSE.
- Project-local `.agents/` folder with the working contract
  and 5 skills (4 from `dotnet/skills`, 1 from
  `devixlabs/DotNetWebApp`).
- Multi-provider persistence scaffolding (SQLite / PostgreSQL /
  MariaDB), with the test matrix decided as **SQLite-only** for
  now and the convention for growing it documented in
  [`../adr/0001-multi-provider-strategy.md`](../adr/0001-multi-provider-strategy.md).
- Radzen.Blazor wired into the Blazor WASM client
  (`AddRadzenComponents()`, providers in `App.razor`, `@using`
  in `_Imports.razor`).
- `dotnet build` green (10/10 projects, 0 errors, 0 warnings).

What's intentionally **not** in this phase:

- No domain entity. No command, no handler, no endpoint, no
  component. The solution is featureless by design.

## 2. Phase 1 — MVP (4–6 weeks)

**Goal:** a single user can sign up, create a workspace,
create a board, add lists and cards, drag cards between
lists, and sign in again tomorrow to see the same state. No
collaboration, no integrations, no AI. This is the smallest
end-to-end vertical that proves the architecture.

### 2.1 Domain & Application

- [ ] `Workspace` aggregate: name, owner, members, board
  collection.
- [ ] `Board` aggregate: name, description, visibility
  (private / public), list collection, member collection,
  label collection.
- [ ] `List` aggregate: name, position, card collection,
  archived state.
- [ ] `Card` aggregate: title, description, position, due date
  (optional), members, labels, checklists, attachments,
  comments, activity events, archived state.
- [ ] `Label` entity: name, color.
- [ ] `Member` entity: user reference + role (admin / member /
  observer).
- [ ] `User` entity: email, display name, avatar, password
  hash, created date, last login.
- [ ] `Attachment` entity: name, mime type, size, blob storage
  reference, uploader.
- [ ] `Checklist` and `ChecklistItem` entities.
- [ ] `Comment` entity.
- [ ] `Activity` entity (append-only event log).
- [ ] Common value objects: `UserId`, `WorkspaceId`, `BoardId`,
  `ListId`, `CardId`, `LabelId`, `Color`, `EmailAddress`,
  `PasswordHash`, `BoardVisibility`, etc.
- [ ] Common errors: `NotFound`, `Forbidden`, `Validation`,
  `Conflict`, etc.

### 2.2 Infrastructure

- [ ] `CardscapeDbContext` with all the entity configurations.
- [ ] Three sets of migrations (one per provider), starting
  with `InitialSchema` and covering the entities above.
- [ ] ASP.NET Identity integration (`UserStore`, `RoleStore`,
  password hashing, etc.).
- [ ] `LocalFileStorageService` for attachments (filesystem,
  configurable root).
- [ ] `AspNetEmailService` that writes to the console in
  Development and to a queue in Production.
- [ ] `DomainEventsInterceptor` that dispatches domain events
  after `SaveChangesAsync`.
- [ ] `AuditableEntityInterceptor` that stamps `CreatedAt`,
  `CreatedBy`, `UpdatedAt`, `UpdatedBy`.

### 2.3 API

- [ ] `AuthEndpoints`: `POST /api/auth/register`,
  `POST /api/auth/login`, `POST /api/auth/refresh`,
  `POST /api/auth/logout`.
- [ ] `WorkspaceEndpoints`: create, get, list, update, delete,
  invite member, accept invite.
- [ ] `BoardEndpoints`: create, get, list (per workspace),
  update (rename, change description / visibility), delete
  (archive), restore from archive, star / unstar.
- [ ] `ListEndpoints`: create, rename, move (reorder), archive,
  restore, move-all-cards.
- [ ] `CardEndpoints`: create, get, update (title, description,
  due date), move (change list + position), archive, restore,
  assign / unassign member, add / remove label, add / remove
  attachment, add / remove comment, add / remove checklist.
- [ ] `LabelEndpoints`: create, rename, recolor, delete.
- [ ] `MemberEndpoints`: list, change role, remove.
- [ ] `SearchEndpoints`: `GET /api/search?q=...` (full-text
  over boards the user has access to).
- [ ] `ActivityEndpoints`: `GET /api/boards/{id}/activity`,
  `GET /api/cards/{id}/activity`.
- [ ] `NotificationEndpoints`: list unread, mark as read.
- [ ] Global exception handling middleware.
- [ ] JWT bearer authentication.
- [ ] CORS for the Blazor dev server.
- [ ] Health checks (`/health/live`, `/health/ready`).
- [ ] Swagger UI in Development.

### 2.4 Web

- [ ] Layout: header (logo, search, notifications bell, user
  menu), left nav (workspaces and boards), main content area.
- [ ] Routes: `/`, `/login`, `/register`, `/workspaces`,
  `/workspaces/{id}`, `/boards/{id}`, `/cards/{id}`,
  `/settings`, `/notifications`.
- [ ] Auth flow: register, login, refresh token, logout.
- [ ] Board view: lists as columns, cards stacked; drag and
  drop with HTML5 DnD.
- [ ] Card detail: tabs for description, comments, checklists,
  attachments, activity.
- [ ] Radzen components throughout (DataGrid for board
  activity, Dialog for create-card, DropDown for label
  picker, DatePicker for due date, FileInput for
  attachments, NotificationService for toasts).
- [ ] Theming: Material theme (the one Radzen ships with).
- [ ] Dark mode toggle (Material-dark).
- [ ] Responsive layout down to tablet (mobile is Phase 5).

### 2.5 Tests

- [ ] Unit tests for every command/query handler.
- [ ] Unit tests for every value object.
- [ ] Unit tests for every domain rule.
- [ ] Integration tests for every endpoint with at least one
  happy path and one error path.
- [ ] Architecture tests (already wired, expand with new
  rules).
- [ ] Functional smoke test: register → create workspace →
  create board → create list → create card → move card →
  archive card.

### 2.6 Release

- `v0.1.0-mvp` tag.
- `docker-compose.yml`: Api + Web + SQLite volume.
- README with a 5-minute quick-start.
- Demo deploy (Railway / Fly.io / a friend's server).

## 3. Phase 2 — Core features (4–6 weeks)

**Goal:** add the features that turn Cardscape from "I can
manage my own tasks" into "I can collaborate with my team".

### 3.1 Collaboration

- [ ] Member invitations via email.
- [ ] Board-level and workspace-level roles with real
  authorization rules.
- [ ] @mentions in card descriptions and comments, with
  notification.
- [ ] Reactions on comments.
- [ ] Card-level watch / un-watch.
- [ ] Email notifications (configurable per-user, per-event).

### 3.2 Card features

- [ ] Card mirror: a single card visible on multiple boards,
  with synced state.
- [ ] Card snooze: hide the card until a date / time.
- [ ] Card repeater: schedule a new copy of the card.
- [ ] Card aging: visual fade of stale cards (configurable
  threshold).
- [ ] Voting on cards.
- [ ] Bulk actions: multi-select, change labels, change due
  date, archive.

### 3.3 Productivity

- [ ] Board templates (workspace-level).
- [ ] Saved filters per board.
- [ ] Quick capture: Inbox endpoint, voice capture via
  browser MediaRecorder API.
- [ ] Command palette (Cmd-K style).
- [ ] Recent boards in the home page.

### 3.4 Real-time

- [ ] SignalR hub per board: live card moves, live comments.
- [ ] Presence indicators on cards and boards.
- [ ] Typing indicators in comments.

### 3.5 Mobile

- [ ] Responsive layout down to phone width.
- [ ] Touch-friendly drag-and-drop.
- [ ] PWA manifest: installable, offline shell.

## 4. Phase 3 — Power-ups & Automation (6–8 weeks)

**Goal:** match 's "extensibility" story without being
a power-up marketplace. We build the most-requested first-party
extensions and a Automation-equivalent automation engine.

### 4.1 Power-up framework

- [ ] Define the `IPowerUp` interface.
- [ ] A power-up is a class registered in DI, identified by
  string ID, with per-board configuration storage.
- [ ] UI: a "Power-Ups" tab in board settings; "Add Power-Up"
  modal listing installed extensions.

### 4.2 First-party extensions

- [ ] **Calendar** — board view showing due-date cards on a
  monthly grid.
- [ ] **Table** — board view showing cards as rows with
  columns from label, due date, members, custom fields.
- [ ] **Timeline** (Gantt) — board view with start / end dates
  on cards.
- [ ] **Dashboard** — board view with cards-per-list, cards-per-member, overdue-counts.
- [ ] **Custom Fields** — schema definition UI, field
  rendering on card back.
- [ ] **Card Aging** — visual fade.
- [ ] **Card Snooze** — UI to schedule a snooze.
- [ ] **Card Repeater** — UI to schedule a repeat.
- [ ] **Voting** — vote button on card front, vote count.
- [ ] **List Limits** — WIP cap.
- [ ] **Dashcards** — count widgets pinned to the board top.

### 4.3 Automation automation

- [ ] Rule engine: trigger (event) → action(s).
- [ ] Triggers: card created, card moved, card archived, due
  date approaching, member added, label added, etc.
- [ ] Actions: move, copy, archive, assign, add label, post
  comment, set due date, mark complete, etc.
- [ ] Custom card buttons (UI on card back).
- [ ] Custom board buttons.
- [ ] Scheduled commands (cron-like, daily / weekly / monthly
  triggers).
- [ ] Per-user quota (configurable; default 250 / month,
  admin-overridable).

### 4.4 Integrations (first-party)

- [ ] **Webhooks** — outgoing POSTs on board events.
- [ ] **iCalendar feed** — per-board `.ics` of due dates.
- [ ] **Slack** — channel notifications on card events.
- [ ] **Google Drive** — file picker in attachments, with
  live preview.
- [ ] **GitHub** — link PRs and issues to cards.
- [ ] **Email-to-board** — inbound email creates a card.

### 4.5 Developer experience

- [ ] OpenAPI spec published to a public URL.
- [ ] Personal access tokens for the REST API.
- [ ] OAuth flow for third-party apps.
- [ ] Webhook signature verification.
- [ ] Public "build your own power-up" guide in
  `docs/extensions/`.

## 5. Phase 4 — Enterprise & AI (6–8 weeks)

**Goal:** the features a 50-person company needs before they
trust Cardscape with their work.

### 5.1 Security

- [ ] OAuth 2.0 / OIDC login (Google, Microsoft, Apple).
- [ ] SAML SSO (enterprise tier).
- [ ] Two-factor authentication (TOTP).
- [ ] API tokens with scopes.
- [ ] Session management: list / revoke active sessions.
- [ ] Audit logs (every administrative action).

### 5.2 Enterprise admin

- [ ] Org-wide permissions.
- [ ] SCIM provisioning.
- [ ] Data residency (per-workspace region).
- [ ] Atlassian-Guard-style policy enforcement.

### 5.3 Inbox & Planner

- [ ] Inbox page (cards captured outside any board).
- [ ] Planner page (personal calendar of due dates).
- [ ] Google Calendar sync (OAuth).
- [ ] Smart scheduling (suggest when to do a card based on
  existing schedule).

### 5.4 AI (Cardscape AI-style)

- [ ] **Provider abstraction** in `Application/Abstractions/IAiService.cs`.
- [ ] **Multiple providers**: a built-in rule-based provider +
  an OpenAI-compatible provider (so users can BYOK).
- [ ] **Card description generation** from the title.
- [ ] **Comment summary**.
- [ ] **Auto-checklist** from the description.
- [ ] **Smart Boards** — AI suggestions: "this card looks
  stale, archive it?", "you might want to assign this to X".

## 6. Phase 5 — Polish & scale (ongoing)

- [ ] Internationalization (i18n): extract all strings into
  resource files; ship English + Spanish, then add more.
- [ ] Theming: dark mode everywhere, custom color themes.
- [ ] Performance: caching (Redis), CDN for attachments.
- [ ] Background jobs: Hangfire for Automation schedules.
- [ ] Rate limiting: per-user and per-IP.
- [ ] Webhooks v2: retry with backoff, dead-letter queue.
- [ ] Mobile native apps (MAUI or PWA-only — TBD).
- [ ] Realtime: presence, typing indicators, live drag.
- [ ] Export: per-board archive download (JSON + attachments).
- [ ] Import:  JSON import (parse
  `boards.cardscape.json` files).
- [ ] API client SDK: ship a C# client, generated from
  OpenAPI via Kiota.
- [ ] Public status page.
- [ ] Security audit (third-party).
- [ ] Pen test.
- [ ] SOC 2 / GDPR compliance docs.
- [ ] Migration tooling from  to Cardscape.

## 7. What we are explicitly NOT building

- **No AI image generation** beyond the Atlassian
  Intelligence-equivalent features. We're not Stable Diffusion
  for boards.
- **No marketplace** for third-party extensions. Power-ups are
  first-party only. We document the IPowerUp interface so
  self-hosters can build their own, but we don't curate or
  distribute them.
- **No billing / subscription**. Cardscape is open-source
  under RPL-1.5; there's no paid tier. The "Enterprise"
  features in Phase 4 are gated by configuration, not by a
  license server.
- **No public cloud version**. We are self-host-first. Cloud
  hosting is up to the user.

## 8. Tracking

We track progress with GitHub issues and milestones, one
milestone per phase. Each issue references the phase it
belongs to and (when applicable) the section of
`00-feature-inventory.md` that motivates it.

The roadmap is reviewed at the end of every phase. New
phases can be added, but the existing phase structure
(0–5) doesn't get reordered — what we already promised
stays in the same order.

## 9. References

- [`00-feature-inventory.md`](00-feature-inventory.md)
  — the source feature list.
- [`../architecture/00-overview.md`](../architecture/00-overview.md)
  — how the solution is shaped.
- [`../development/02-vertical-slices.md`](../development/02-vertical-slices.md)
  — the recipe for adding a feature end-to-end.
- [`../adr/0001-multi-provider-strategy.md`](../adr/0001-multi-provider-strategy.md)
  — the multi-provider / SQLite-only decision.
