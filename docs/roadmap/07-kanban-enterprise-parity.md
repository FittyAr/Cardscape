# Kanban Enterprise parity — what's worth building, what's not

> The maintainer's take on the **Kanban Enterprise** feature
...
>    small teams, and small-and-medium companies?** (Kanban's
...
>
> The hard call: most of the Kanban Enterprise surface is
...
So the doc partitions the Kanban Enterprise surface into
three buckets:
...
- **C — Checklist, not for tomorrow.** Looks good in a
  "we are feature-parity with Kanban" table, but the
...
The Kanban Enterprise surface is grouped by the **user
problem** it solves, not by the **Kanban pricing page**,
...
| **Domain capture / auto-claim** | **C** | — | Kanban uses this to lock an organisation to a domain (anyone with `@acme.com` on Kanban auto-joins Acme's enterprise). Useful for **multi-tenant SaaS**, useless for a self-hoster (the self-hoster IS the only tenant). A single-user OSS deployment does not have multiple companies to keep separate. Skip. |
...
| **Multi-org / multi-workspace consolidation** | **C** | — | Kanban lets an enterprise roll up 20+ workspaces into a single billing/admin view. The self-hoster equivalent is "I have one Cardscape instance, why do I need a multi-org layer?" The self-hosted version of the problem is solved by `Workspace` already. Skip. |
...
| **Board-level role overrides** | **B** | v1.3.0+ | Kanban has `Admin` / `Editor` / `Observer` per board. Cardscape inherits role from the workspace today. The override is a small DTO and a handler change, but the UI surface is non-trivial (every member-list in every board needs a per-row role chip). Track for v1.3.0. |
...
| **Custom branding** (org logo, color, login screen) | **C** | — | Real demand from resellers / agencies; zero demand from a self-hoster who IS the org. Kanban has it because the Enterprise buyer has a brand team. Cardscape's branding is the project branding, not per-deployment. Skip. |
...
| **Status page** (Kanban-style "all systems operational") | **A (done)** | shipped | Live at the deployment's `/status` route. Per `docs/operations/05-status-page-deploy.md`. |
...
The doc lives at `docs/roadmap/07-kanban-enterprise-parity.md`
