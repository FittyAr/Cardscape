...
- [ ] Persistencia: modelo EF, transacciones, concurrencia, índices, consultas N+1, tracking y compatibilidad de los tres providers. El núcleo Board/List/Card y sus capacidades de aging, snooze, recurrence, mirrors, voting, checklists, comments y attachments ya filtran/agregan mediante EF Core; mirrors dejó de ejecutar N+1. Sólo SQLite conserva en cliente comparaciones/orden `DateTimeOffset` no traducibles. Continúan otros repositorios auxiliares.
