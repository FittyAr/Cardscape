...
- [ ] Persistencia: modelo EF, transacciones, concurrencia, índices, consultas N+1, tracking y compatibilidad de los tres providers. El núcleo Board/List/Card, sus capacidades auxiliares y las integraciones GitHub/Slack/inbound-email/webhooks ya filtran y agregan mediante EF Core; mirrors dejó de ejecutar N+1. Sólo SQLite conserva en cliente comparaciones/orden `DateTimeOffset` no traducibles y los tokens CSV mantienen su coincidencia exacta tras un prefiltro SQL. Continúan identidad, actividad, automatizaciones y custom fields.
