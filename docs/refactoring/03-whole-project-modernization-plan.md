...
- Mantener SQLite como único provider soportado hasta que otro engine entregue en el mismo bloque migraciones aisladas, matriz automática y despliegue real.
...
- [x] La estrategia multi-provider ficticia fue reemplazada por SQLite-only: selector, paquetes y excepciones muertas retirados; migraciones, runtime, CI, Docker y documentación normativa usan el mismo provider.
...
- [ ] Persistencia: modelo EF, transacciones, concurrencia, índices, consultas N+1 y tracking sobre SQLite. Todos los repositorios y la exportación personal ya filtran, ordenan, paginan y agregan mediante LINQ/EF Core; mirrors dejó de ejecutar N+1 y las mutaciones masivas usan `ExecuteUpdateAsync`/`ExecuteDeleteAsync`. No hay SQL manual en este recorrido. Los índices de actividad, jobs, notificaciones, invitaciones, tokens, automatizaciones, custom fields, labels y colecciones de card ya siguen sus filtros/órdenes reales y se retiraron prefijos redundantes. SQLite conserva en cliente sólo comparaciones/orden `DateTimeOffset` no traducibles, SCIM verifica localmente hashes con sal tras filtrar activos mediante EF Core y los tokens CSV mantienen su coincidencia exacta tras un prefiltro EF Core. El runtime, las migraciones, CI, Docker y la documentación ya declaran un único provider real; continúan transacciones y concurrencia.
...
| 2026-08-22 | Persistencia SQLite-only honesta | Retirados selector runtime, Npgsql/MySQL, ramas de excepción, configuración, scripts y documentación de providers sin migraciones, CI ni despliegue reales; ADR 0012 fija el único límite soportado y 0 SQL manual | Build 0/0; modelo EF sin cambios pendientes; grafo sólo SQLite; suite 901 pass, 0 fail, 1 skip | Incluido en este commit |
