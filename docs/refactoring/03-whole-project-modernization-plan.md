...
- [ ] Persistencia: modelo EF, transacciones, concurrencia, índices, consultas N+1, tracking y compatibilidad de los tres providers. El núcleo Board/List/Card ya filtra, une, ordena y proyecta con EF Core server-side; sólo SQLite conserva en cliente el rango/orden `DateTimeOffset` que su provider no traduce. Continúa la eliminación de scans en repositorios auxiliares.
